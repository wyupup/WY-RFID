...
#define wipeB 5     // Button pin for WipeMode
...
//  digitalWrite(relay, HIGH);    // Make sure door is locked
  ...
  lcd.print("wy RFID v1.0");
  ShowReaderDetails();  // Show details of PCD - MFRC522 Card Reader details

  //Wipe Code - If the Button (wipeB) Pressed while setup run (powered on) it wipes EEPROM
  if (digitalRead(wipeB) == LOW) {  // when button pressed pin should get low, button connected to ground
    digitalWrite(redLed, LED_OFF);// Red Led stays on to inform user we are going to wipe
     lcd.clear();
     lcd.print("wipe");
  ...
  for ( uint8_t i = 0; i < 4; i++ ) {          // Read Master Card's UID from EEPROM
    ...
    Serial.print(masterCard[i], HEX);
        ...
        lcd.clear();
        lcd.print("main card get");
        delay(2000);
        lcd.clear();
        lcd.print("wait for you");
  Serial.println("");
  Serial.println(F("-------------------"));
...
void loop () {
  do {
    ...
    if (digitalRead(wipeB) == LOW) { // Check if button is pressed
      ...
      lcd.clear();
      lcd.setCursor(0,0); 
      lcd.print("10s wiped,loosen ");
      lcd.setCursor(0,1);
      lcd.print("for cancle");
      ...
      if (buttonState == true && digitalRead(wipeB) == LOW) {    // If button still be pressed, wipe EEPROM
        ...
        lcd.clear();
        lcd.setCursor(0,0); 
        lcd.print("wiped sucessful");
        lcd.setCursor(0,1);
        lcd.print("");
        delay(2000);
        lcd.clear();
        lcd.setCursor(0,0); 
        lcd.print("reset for new");
        lcd.setCursor(0,1);
        lcd.print("main card");
  ...
  if (programMode) {
    if ( isMaster(readCard) ) { //When in program mode check First If master card scanned again to exit program mode
      ...
      if ( findID(readCard) ) { // If scanned card is known delete it
        ...
        lcd.clear();
        lcd.print("delete sucessful");
        delay(2000);
        lcd.clear();
        lcd.setCursor(0,0);
        lcd.print("put main card");
        lcd.setCursor(0,1);
        lcd.print("to common mode");
...
void successDelete() {
  ...
  digitalWrite(greenLed, LED_ON);  // Make sure green LED is off
  delay(200);
  digitalWrite(redLed, LED_OFF);  // Make sure blue LED is on
  delay(200);
